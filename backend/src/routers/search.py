...
async def global_search(q: str = Query("", min_length=1), db=Depends(get_db)):
    ...
    articles_db = (
        db.query(Article)
        .filter(Article.title.ilike(term) | Article.summary.ilike(term))
        .all()
    )
    exp_db = (
        db.query(Experience)
        .filter(Experience.company_name.ilike(term) | Experience.title.ilike(term))
        .all()
    )
    ...
    return GlobalSearchResponse(
        query=q,
        projects=[
            SearchResultItem(id=p.id, title=p.name, subtitle=p.description, url=p.url)
        ...
        experiences=[
            SearchResultItem(
                id=e.id, title=f"{e.title} at {e.company_name}", subtitle=e.description
            )
            for e in exp_db
        ],
        certificates=[
